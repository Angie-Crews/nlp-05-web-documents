"""
src/nlp/stage03_transform_crews_p4.py - Transform Stage (Crews Phase 4)

Transforms validated BeautifulSoup object into a structured DataFrame for your crews phase 4 pipeline.
"""
...
def run_transform(
    ...
    LOG.info("========================")
    LOG.info("STAGE 03: TRANSFORM starting (crews_p4)...")
    LOG.info("========================")
    ...
    # Extract subjects from the metatable (robust for arXiv)
    ...
    if primary_subject_span:
        ...
        if match:
            primary_subject_code = match.group(1)
        else:
    ...
    # Extract PDF link (option 1, fixed for arXiv)
    ...
    # Debug: log the subjects string being parsed
    LOG.info(f"DEBUG: subjects field raw value: {subjects}")
    # (extraction now handled above)
    record = {
        "arxiv_id": arxiv_id,
        "title": title,
        "authors": authors,
        ...
        "crews_p4_pdf_url": pdf_url,
        "crews_p4_primary_subject_code": primary_subject_code,
    }
